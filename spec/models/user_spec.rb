...
describe User do
  ...
  it "should have a method 'active?' which will return true if user is activated otherwise false" do
    users(:impressions).active?.should be_true
  ...
  it "should be able to authenticate a user with correct email and password" do
    user = User.authenticate('23impressions@gmail.com', 'monkey')
    user.should_not be_nil
    user.should eql users(:impressions)
  end
  
  it "should not authenticate if email or password are blank" do
    user = User.authenticate('23impressions@gmail.com', '')
    user.should be_nil
  ...
  it "should not authenticate with invalid email" do
    user = User.authenticate('some_invalid_email@23impressions.com', 'monkey')
    user.should be_nil
  end
  
  it "should not authenticate inactive user" do
    user = User.authenticate('23impressions@gmail.com.com', 'monkey')
    user.should be_nil
  end
  
  it "should have a method which will convert all upcase character to downcase for email" do
    upcase_email = "23impressions@gmail.com"
    user = User.new
  ...
  it "should be able to validate role" do
    ...
    users(:impressions).has_role?(:user).should be_true
    users(:impressions).has_role?(:admin).should be_false
  end
